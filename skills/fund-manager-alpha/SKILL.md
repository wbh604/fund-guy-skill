...
| 脚本 | 定位 |
|---|---|
| `fetch_fund.py` / `fetch_stock_klines.py` / `fetch_house.py` / `fetch_pingzhong.py` / `fetch_top_funds.py` / `fetch_gates.py` | 取数参考:展示接口用法与缓存结构。接口失效时**你自己按三层模型取数**,不要修脚本修到天荒地老 |
| `analyze_fund.py` / `analyze_house.py` / `analyze_gates.py` / `build_replay_data.py` | 纯计算参考:事件推断/被动减仓判定/验尸/House分歧/闸门分位的算法定义,输入齐了就是确定性计算,可直接复用 |
| `build_fund_report.py` / `build_v2.py` | 渲染参考:设计系统与模块组装 |
| `export_cards.py` | 分享卡截图 |

**判断类的工作(公告解读、Mandate 抽取、事件核实、稀缺度定级)没有脚本,也永远不该有。**
