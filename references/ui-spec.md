...
**评分锚定行为,不锚定净值**(2026-08-11 定稿,用户原则):

总评分 = **择时能力 35% + 控制能力 35% + 超额质量 30%**,三轴都从行为与统计证据算出,规则透明可复算:

| 轴 | 证据 | 计算 |
|---|---|---|
| 择时能力 | 买卖点验尸:每个买/卖动作拿 **12 个月后的走势**验 | 买点胜率(超额口径)×0.4 + 卖点躲跌率×0.4 + TM 回归 γ 显著性(10) + 仓位调向正确率(10) |
| 控制能力 | 熊市防守 + 极端月 + 处置纪律 | 熊市年均防守超额(0-40) + 大盘最差10月超额(0-30) + 下行捕获<100(0-30) |
| 超额质量 | 信息比率 | min(100, IR×100) |

前端呈现:
- 判决书评分卡 = 巨型总分 + 三条轴分横条(择时弱项标 ⚠ 橙色)
- 买卖复盘 section 末尾接**买卖点验尸**:双条(买点胜率/卖点躲跌率) + 一句判词(如"会买 · 不会卖") + 最好/最差买卖点四张卡(🏆/💀,含 12M 后走势数字) + 熊市防守表 + 浮亏处置对比大数字(止损 N 次 vs 越跌越买 N 次) + TM γ 徽章
- 净值只作为证据输入,不作为评分对象 —— "评的是行为,不是净值"直接写在评分卡标签里
