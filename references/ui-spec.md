...
| 轴 | 证据 | 计算 |
|---|---|---|
| 择时能力 | 买卖点验尸:每个买/卖动作拿 **12 个月后的走势**验;**被动减仓先剔除** | 买点胜率(超额口径)×0.4 + 卖点躲跌率×0.4 + TM 回归 γ 显著性(10) + 仓位调向正确率(10) |
...
**被动减仓判定(硬规则,2026-08-11)**:减仓不一定是决策,两类被动情形必须识别并剔除:

| 类型 | 判定 | 标记 |
|---|---|---|
| **触双十线** | 上季该股占净值 ≥9.3%(逼近 10% 监管线)后出现减持 | `被动减·触线` |
| **遭赎回** | 基金份额估算同期缩水 >12%,且该股减幅 ≤ 份额缩水 +20pp(等比例被动卖) | `被动减·赎回` |

- 份额估算 = 季度规模(Σ市值/Σ占比) ÷ 单位净值
- 被动减仓**不进卖点验尸、不计入择时评分、不算止损纪律**;图上用琥珀色小点区分,回放字幕标「被动 · 非主动决策」
- 报告必须展示剔除数量("被动减仓已剔除 N 次"),否则躲跌率是失真的
