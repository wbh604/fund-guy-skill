...
| **高位圈钱** | 趁行情好猛发新基金 | 市场/风格估值高位期新增产品 → 红旗 | 🟡 需判断当时估值分位 |
| **年底冲排名** | 为名次年末赌一把 | Q4 仓位或热门板块暴露显著高于其他季度 → 红旗 | 🟢 可自动算 Q4 vs 非 Q4 差异 |
...
这是个人投资者特别关心、但绝大多数基金研究完全不回答的问题。**每个经理都必须算两遍收益:从经理时点算一次,从披露时点算一次。**
...
| 类型 | 例子 | 主要来源 |
|---|---|---|
| **宏观 macro** | 贸易战开打 / 疫情爆发封城 / 俄乌战争 / 对等关税 | 新闻检索(标题+日期可查证) |
| **市场 market** | 抱团股见顶 / 美股熔断 / AI 行情引爆 / 微盘流动性危机 / 924 政策暴涨 | 新闻检索 + 指数行情反推(单日 ±5% 自动标注) |
| **基金自身 fund** | 一日售罄上热搜 / 开始限购 / 经理变更公告 / 规模异动 / 上热搜被骂 | 基金公告 + 新闻检索 |
...
这个 Skill 没有 fetcher 脚本,也不要去造一个。原因很实际:
...
所以:**你(agent)负责取数和判断,脚本只做纯计算**。

### 取数方式 —— 优先 akshare,判断仍归 agent
...
| 限购公告 | `fund_announcement_personnel_em` 邻接口 | Career Incentive 证据 |
...
- **失败不重试超过 3 次** —— 第 3 次还失败就人工介入,不要无限循环触发封禁
- 大批量任务(>50 个标的)改走 `fund_report_stock_cninfo` 横截面,一次拿全市场

### 脚本的位置

`scripts/` 下的东西都是**参考实现**,能用就用,不合适就自己算:

| 脚本 | 干什么 |
|---|---|
| `export_cards.py` | 把报告页里的分享卡截成 PNG |
| (可选) 因子回归、Divergence 计算 | 纯数学,输入齐了就是确定性计算 |
